...
export default function BriefTab() {
  ...
  useEffect(() => {
    supabase.from('event_config').select('value').eq('key', 'race_brief')
      .then(({ data }) => {
        if (data && data.length > 0) setText(data[0].value || '')
        setLoaded(true)
      })
  }, [])
  ...
  if (!loaded) return (
    <div style={{ padding: 32, textAlign: 'center', color: 'rgba(255,255,255,0.3)', fontSize: 13 }}>Loading…</div>
  )
    ...
    <div>
      <Card style={{ marginBottom: 14 }}>
        ...
        <div style={{ padding: '12px 16px', fontSize: 13, color: 'rgba(255,255,255,0.4)', lineHeight: 1.5 }}>
          Edit the race morning brief below. Saved to Supabase — visible to anyone with the link.
        </div>
      </Card>
